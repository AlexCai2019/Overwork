...
public class ColorOperation implements IHasDestructor
{
	...
	ColorOperation()
	{
		...
		String remainTimeString = jsonCore.get(TimeType.remainTime, JSONHelper.COLOR, "0", String.class); //剩餘時間
		String passTimeString = jsonCore.get(TimeType.passTime, JSONHelper.COLOR, "0", String.class); //經過時間
	...
	{
		JSONHelper jsonCore = JSONHelper.getInstance(); //JSON處理核心
		jsonCore.put(TimeType.remainTime, JSONHelper.COLOR, String.format("%06X", remainTimeColor));
		jsonCore.put(TimeType.passTime, JSONHelper.COLOR, String.format("%06X", passTimeColor));
	}
}